  ...
  <div>
    ...
    <b-modal v-if="showPermit" v-model="showPermit" size="xl" id="species-modal" title="Estado del Permiso" hide-footer> 
      <div class="ma-5 d-flex justify-content-between align-items-center">
        ...
        <b-badge v-if="selectedPermit.status === 'not_valid'" class="p-2" variant="danger">Corrige los errores indicados</b-badge>
      </div>
      ...
      <b-row>
        ...
        <b-col class="pl-4" sm="12" lg="3">{{columns[1]}}</b-col>
        <b-col class="pl-4" sm="12" lg="3">{{columns[2]}}</b-col>
      </b-row>
      <b-row class="my-3" v-for="(requeriment, index) of selectedPermit.requeriments" v-bind:key="index">
        ...
        <b-col sm="12" lg="3">
          <div>
            ...
            <b-badge v-if="requeriment.pivot.is_valid === null" class="p-2" variant="danger">
              No checkeado
              <font-awesome-icon :icon="['fa', 'clipboard']"></font-awesome-icon>
            </b-badge>
            <b-badge v-if="(!requeriment.pivot.is_valid && requeriment.pivot.is_valid !== null)" class="p-2" variant="danger">
              No valido
              <font-awesome-icon :icon="['fa', 'ban']"></font-awesome-icon>
            </b-badge>
          </div>
          <div v-if="(!requeriment.pivot.is_valid && requeriment.pivot.is_valid !== null)">
