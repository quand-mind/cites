<template>
  <div>
    <b-row class="mt-3">
      ...
      <b-col lg="3" class="w-100">Cantidad:</b-col>
      <b-col lg="3" v-if="type === 'client'" class="w-100">Acciones:</b-col>
      <b-col lg="1" v-if="type !== 'client'" class="w-100">Acciones:</b-col>
      <b-col lg="2" v-if="type !== 'client'" class="w-100">Archivo:</b-col>
    </b-row>
    <b-row class="mt-3 mb-3" v-for="(specie, index) of selectedSpecies" v-bind:key="index">
      ...
      <b-col lg="3" class="w-100">{{specie.qty}}</b-col>
      <b-col lg="3" class="w-100" v-if="type !== 'client'">
        <a v-if="specie.legal_document" :href="specie.path" target="_blank"><font-awesome-icon :icon="['fa', 'eye']"></font-awesome-icon> Archivo ({{ specie.legal_document.name }})</a>
        <button v-if="specie.legal_document" class="ml-3 btn text-danger relative" @click.prevent="deleteFile(specie)" style="cursor:pointer">
          <font-awesome-icon :icon="['fa', 'trash']" @click.prevent="deleteFile(specie)"></font-awesome-icon>
        </button>
